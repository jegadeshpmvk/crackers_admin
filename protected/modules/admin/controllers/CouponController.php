...
class CouponController extends Controller
{
    ...
    protected function renderForm($model)
    {
        if ($model->load(Yii::$app->request->post())) {
            ...
            if ($model->save()) {
                Yii::$app->session->setFlash('success', 'Coupon details were saved successfully.');
    ...
    public function actionExportExcel()
    {
        ...
        $Category = Coupon::find()->select(['id'])->all();
        if (count($Category) > 0) {
            foreach (Coupon::find()->each(10) as $m) {
        ...
        $fname = 'Coupon_' . date('d_m_Y_H_i_s');
    ...
    public function actionImportExcel()
    {
        ...
        foreach ($sheetData as $row) {
            ...
            // Update or Create
            $model = !empty($row[0]) ? Coupon::findOne($row[0]) : new Coupon();


            $model->name = trim((string)$row[1] ?? '');
            $model->code = trim((string)$row[2] ?? '');
            $model->discount = trim((string)$row[3] ?? '');
